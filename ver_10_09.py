...
class SpeechRecognitionThread(QThread):
    ...
    def run(self):
        ...
        with mic as source:
            print("Listening...")
            recognizer.adjust_for_ambient_noise(source)  # �ֺ� ���� ����
            try:
                audio = recognizer.listen(source, timeout=5)  # timeout �߰�
                recognized_text = recognizer.recognize_google(audio, language="ko-KR")
                print(f"Recognized Speech: {recognized_text}")
                self.recognized_signal.emit(recognized_text)
            except sr.WaitTimeoutError:
                print("Listening timed out while waiting for phrase to start")
                self.recognized_signal.emit("")
...
class ActuatorController:
    ...
    def control_actuator(self):
        """액추에이터 제어 로직"""
        if self.switch_state["switch_1"] == GPIO.LOW:
            print("스위치 상태: 2 (전진)")
            self.forward_actuator()
        elif self.switch_state["switch_0"] == GPIO.LOW:
            if self.hall_sensor_state == GPIO.LOW:
    ...
    def actuator_loop(self):
        """액추에이터 제어 쓰레드"""
        while self.running:
            self.read_inputs()  # 입력 상태 읽기
            self.control_actuator()  # 액추에이터 제어
            time.sleep(0.1)  # 100ms 대기

    def start(self):
        """쓰레드 시작"""
        self.thread = Thread(target=self.actuator_loop)
        self.thread.start()
...
class Ui_MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.pushButton_speech.clicked.connect(self.start_speech_recognition)

        # ���̺� �ʱ�ȭ �κ�
        self.miclabel = QLabel()  # ���� ���̺�, ���� �ڵ忡 �°� �ʱ�ȭ

        self.speech_recognition_thread = SpeechRecognitionThread()
        self.speech_recognition_thread.recognized_signal.connect(self.handle_recognized_text)
        # Set up GPS worker
    ...
    def initUI(self):
        ...
        self.webView = QWebView(self.page)
        self.webView.setGeometry(310, 90, 691, 471)
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.refresh_page)  # 타이머가 동작할 때 호출할 메서드 연결
        self.timer.start(3000)  # 3000ms = 3초 간격으로 반복 실행
    ...
    def handle_recognized_text(self, recognized_text):
        ...
        if recognized_text == "초기상태":
            ...
            initialize_label(self.statelabel_1, "1단계", 30, background_color="#6DC9FF")
            initialize_label(self.statelabel_2, "1단계", 45, background_color="#6DC9FF")
            self.speed_1_status()
        elif recognized_text == "2?�계":
            initialize_label(self.statelabel_1, "?�도 : 2?�계", 30, background_color="#5271FF")
            initialize_label(self.statelabel_2, "?�도 : 2?�계", 45, background_color="#5271FF")
            self.speed_2_status()
        elif recognized_text == "조이?�틱 모드" or recognized_text == "조이?�틱":
            initialize_label(self.statelabel_1, "조이?�틱 모드", 25, background_color="#FF5757")
            initialize_label(self.statelabel_2, "조이?�틱 모드", 40, background_color="#FF5757")
            self.toggle_joystick_mode()
        elif recognized_text == "?�유 모드" or recognized_text == "?�유":
            initialize_label(self.statelabel_1, "?�유 모드", 30, background_color="#FFDE5A")
            initialize_label(self.statelabel_2, "?�유 모드", 45, background_color="#FFDE5A")
            self.speed_3_status()
